...
- [ ] Настройка CI/CD пайплайна (builds, unit-тесты, интеграционные тесты)
- [ ] Публичный трекер задач (Savannah / GitLab)
- [ ] Система RFC (Request for Comments) для архитектурных решений
- [ ] Кодекс участника (Code of Conduct)
- [ ] Документация для контрибьюторов (`CONTRIBUTING.md`)
- [ ] Настройка mailing list (GNU Mailman)

#### 0.2 Кросс-компилятор и сборочная среда

- [ ] Сборка кросс-компилятора `binutils` для целевых архитектур
- [ ] Сборка кросс-компилятора `GCC` (C, C++) под `x86_64-gnuos-elf`
- [ ] Настройка `QEMU` для эмуляции всех целевых платформ
- [ ] Образ Docker / контейнер для воспроизводимой сборки
- [ ] Скрипты автоматической настройки окружения разработчика

#### 0.3 Инструменты качества

- [ ] Статический анализатор (cppcheck, clang-tidy)
- [ ] Форматтер кода (`clang-format`, `.editorconfig`)
- [ ] Покрытие кода (gcov / lcov)
- [ ] Fuzzing-инфраструктура (AFL++, libFuzzer)
- [ ] Политика review: минимум 2 approve перед merge

#### 0.4 Документация

- [ ] Архитектурный документ (Architecture Decision Records — ADR)
- [ ] Стандарты кодирования (GNU Coding Standards + расширения)
...
- [ ] Поддержка Multiboot2 протокола (совместимость с GRUB 2)
- [ ] Настройка GDT (Global Descriptor Table) для x86_64
- [ ] Переход в 64-битный защищённый режим (Long Mode)
- [ ] Базовая инициализация стека
- [ ] Передача управления ядру с картой памяти
- [ ] Поддержка UEFI (UEFI stub loader)
- [ ] Настройка framebuffer для раннего вывода (VESA / GOP)

#### 1.2 Начальная инициализация ядра

- [ ] Раннее управление памятью (физическим аллокатором страниц)
- [ ] Настройка IDT (Interrupt Descriptor Table)
- [ ] Обработчики исключений CPU (page fault, GPF, #DE, ...)
- [ ] Настройка APIC / xAPIC / x2APIC
- [ ] Базовый вывод через VGA/serial для отладки (`kprintf`)
...
- [ ] 4-уровневые таблицы страниц (PML4) для x86_64
- [ ] Kernel virtual address space layout (KASLR-ready)
- [ ] Аллокатор виртуального адресного пространства
- [ ] Поддержка huge pages (2 МБ, 1 ГБ)
...
- [ ] Базовые структуры: Task Control Block (TCB), Process Control Block (PCB)
...
- [ ] Мьютексы, спинлоки, RW-локи
- [ ] Атомарные операции (на базе `__atomic_*`)
- [ ] Очереди ожидания (wait queues)
- [ ] Механизм work queues для отложенных задач
- [ ] Поддержка RCU (Read-Copy-Update)

#### 1.8 Микроядерный IPC

- [ ] Синхронная передача сообщений (rendezvous)
- [ ] Асинхронные очереди сообщений
- [ ] Передача capability (файловые дескрипторы, права доступа)
- [ ] Разделяемая память между процессами (SHM)
- [ ] Именованные каналы ядра

#### 1.9 Драйверы низкого уровня

- [ ] Последовательный порт (UART 16550) — отладочный вывод
- [ ] PS/2 клавиатура (базовая)
- [ ] PCI / PCIe сканирование и перечисление устройств
- [ ] MSI / MSI-X прерывания
- [ ] DMA engine (базовый)
...
- [ ] Таблица syscall (syscall_table) с номерами, совместимыми с Linux ABI (опционально)
- [ ] Вход/выход через `syscall`/`sysret` (x86_64 fast path)
- [ ] Валидация пользовательских указателей перед разыменованием
